...
function ChatComponent({ setEdit }: chatComponentProps) {
  const [_defaultPrompt, setDefaultPrompt] = useState<string>("");
  ...
  async function handleGetSummary() {
    ...
    try {
      ...
      const response = await new Promise<{ error?: string; result?: string }>(
        (resolve) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-col gap-y-4 justify-end mb-4 w-full">
        <Separator className="border-2" />
        <div className="">
          <Button
            disabled={loading}
            onClick={handleGetSummary}
            className="w-full bg-green-600 text-white hover:bg-green-800 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? <ThreeDotLoader /> : "Get Summary"}
          </Button>
        </div>
      </div>
    </div>
